...
from schema.user import AllUser, ModifyUser, NewUser, UserBase
...
def me(authorization: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    user_dict = {
        ...
        "menus": ["system-manage", "user-manage", "record-manage", "user-add"],
    }
...
def get_user_info(username: str, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.username == username).first()
    logger.info(f"获取到的用护: email{user}")
    if user:
        user_dict = {
            "username": user.username,
            "email": user.email,
...
def all_users(
    ...
    if search_username:
        ...
        users = (
            db.query(User)
            .filter(User.username == search_username)
            .slice(page_size * (page_no - 1), page_size * page_no)
        )
    else:
...
def create_user(
    user: NewUser,
    request: Request,
    operator: Optional[str] = Header(None),
    db: Session = Depends(get_db),
):
